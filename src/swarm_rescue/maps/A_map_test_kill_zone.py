...
Some parts of the file like map_test_kill_zone are replaced when a map is generated
"""
...
from spg_overlay.entities.sensor_disablers import ZoneType, NoGpsZone, srdisabler_disables_device, KillZone
...
from .A_walls_map_test_kill_zone import add_walls, add_boxes


class MapTestKillZone(MapAbstract):
    ...
    def __init__(self, zones_config: ZonesConfig = ()):
        ...
        # PARAMETERS MAP
        self._size_area = (628, 432)

        self._rescue_center = RescueCenter(size=(139,67))
        self._rescue_center_pos = ((236,170), 0)

        # self._no_gps_zone = NoGpsZone(size=(400, 500))
        # self._no_gps_zone_pos = ((-190, 0), 0)

        self._kill_zone = KillZone(size=(100, 200))
        self._kill_zone_pos = ((-100, -10), 0)

        self._wounded_persons_pos = [(-263,-8)]
        self._number_wounded_persons = len(self._wounded_persons_pos)
        self._wounded_persons: List[WoundedPerson] = []

        orient = random.uniform(-math.pi, math.pi)
        self._drones_pos = [((240, 118), orient), ((280, 118), orient)]
