"""
This file is a modified copy of swarm rescue original code image_to_map.py
This file runs the ImageToMap.launch() code on a map provided in map_data folder
and imports the generated code to a new map
"""
...
class ImageToMap:
    ...
    def detect_people(self):
        ...
        txt_people_position = "["  # modified from original swarm rescue code
        ...
        self.wounded_persons_pos_text = txt_people_position
    ...
    def detect_rescue_center(self):
        ...
        self.txt_rescue1 = "        self._rescue_center = RescueCenter(size=({0:.0f},{1:.0f}))".format(w_rec, h_rec)  # modified from original swarm rescue code
        self.txt_rescue2 = "        self._rescue_center_pos = (({0:.0f},{1:.0f}), 0)".format(x, y)  # modified from original swarm rescue code
        # print(self.txt_rescue1)
        # print(self.txt_rescue2)
...
map_name = input("!! Map name should be the name of the png map file in map_data folder without \"A_\" at beginning !!\nExemple : for \"A_map_inter_01.png\" file enter \"map_inter_01\"\nEnter map name : ")
img_path = "../../../map_data/A_" + map_name + ".png"
...
#region copy generated walls file to maps folder
f_source = open("generated_code.py", "r")
f_dest = open("../maps/A_walls_" + map_name + ".py", "w")
commented_line = False
for line in f_source:
    if line[:3] == "\"\"\"":
        commented_line = not commented_line
        continue
...
print("Walls file generated in \"../maps/A_walls_" + map_name + ".py\"")
#endregion

#region create map file from A_empty_map.py
empty_map_file = open("A_empty_map.py", "r")
map_file = open("../maps/A_" + map_name + ".py", "w")
map_class_name = "".join([word[0].capitalize()+word[1:] for word in map_name.split("_")])
for line in empty_map_file:
    line = line.replace("*MAP_NAME*", map_name)
    line = line.replace("*MAP_CLASS_NAME*", map_class_name)
    line = line.replace("*SIZE_AREA*", f"({image_to_map.width_map}, {image_to_map.height_map})")
    line = line.replace("*WOUNDED_PERSONS_POS*", image_to_map.wounded_persons_pos_text)
    line = line.replace("*RESCUE_CENTER_CODE*", image_to_map.txt_rescue1+'\n'+image_to_map.txt_rescue2)
    map_file.write(line)
print("Map file generated in \"../maps/A_" + map_name + ".py\"")
#endregion

#region copy map import to clipboard
import os
import_text = "from maps.A_"+map_name+" import " + map_class_name
command = 'echo ' + import_text.strip() + '| clip'
os.system(command)
print("Map import code \""+import_text+"\" copied to clipboard")
#endregion

print("!!! Drones positions may be modified in the map file !!!")
